...
\begin{frame}[t]{}
	\begin{center}
		\includegraphics[width=0.9\linewidth]{images/gpt3i}
	\end{center}
	\note{
...
\begin{frame}{Importance of Search}
	\begin{columns}
		...
		\end{column}
	\end{columns}
	\blfootnote{\url{https://x.com/polynoamial/status/1840822629625688469}}
\end{frame}
...
\begin{frame}{Outline}
	\tableofcontents[hideallsubsections, currentsubsection]
\end{frame}
...
\begin{frame}{Assumption: Verifier}
	...
	\begin{itemize}
		...
		\item Unit test for code \cite{Hendrycks2021-mz}
...
\begin{frame}{Suspect 1: Guess + Check}
	\begin{columns}
		...
		\end{column}
		\begin{column}{0.5\linewidth}
			\includegraphics[width=0.9\textwidth]{images/reject1}
		\end{column}
	\end{columns}
\end{frame}
...
\begin{frame}{Framework: Rejection Sampling EM}
	...
	\end{itemize}
	\blfootnote{\cite{Lardner1989-ev,Hinton1993-vm}}
\end{frame}
		...
		\begin{column}{0.5\linewidth}
			...
			\begin{enumerate}
				\item Sample next step,
				      $$\cfz{z_t} \sim p(\cdot | \cx, \cfz{z_{1:t-1}}^n)$$
...
\begin{frame}{Beam Search with Roll-Outs}
	\begin{columns}
		\begin{column}{0.5\linewidth}
			For a $\cfz{z_{t}}$, sample answers $$\cy{y}^{n} \sim p(\cdot|\cx,\cfz{z_{1:t-1}})$$
			$\textcolor{c2}{r_{MC}}(\cfz{z_{t}}) = \frac{1}{N}\sum_{i=1}^{n} \Ver(\cy^{i})$
		\end{column}
		\begin{column}{0.5\linewidth}
...
\begin{frame}{Interleaved Value Functions}
	\begin{columns}
		\begin{column}{0.5\linewidth}
			\begin{itemize}
...
\begin{frame}{Framework: Expert Iteration}
	...
	\begin{itemize}
		...
		\item Label samples using $\Verm_x(\cy)$
...
\begin{frame}{Review: MCTS}
	...
	\begin{center}
		\foreach \i in {10,...,30}{
				\only<+>{\includegraphics[width=\textwidth]{images/mcts0\i}}
			}
	\end{center}
\end{frame}
...
\begin{frame}{UCB for Language}
	\begin{itemize}
		...
		\item Expand: Sample $N$ next steps $\cfz{z_t}^n$, pick one at random
		\item Rollouts: Sample steps $\cfz{z_{t+1}} \ldots \cfz{z_T}$
		\item Backprop: Update nodes counts $z_{1:t}$ based on results
	\end{itemize}
	\blfootnote{\cite{Feng2023-sz}}
\end{frame}
