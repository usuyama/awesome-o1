...
\begin{frame}{Beam Search with Roll-Outs}
    % The notation is a bit much here, but I wanted to make sure everything had separate indices.
    % Also we get the intermediate chain of thought and solutions since we're sampling from the joint and
    % not the marginal, but I don't mention that since we only need the solution for the verifier.
	\begin{columns}
		\begin{column}{0.5\linewidth}
			\begin{itemize}
			    \item For one $\tilde{z}^{i}_{t}$, sample $n$ solutions $\tilde{y}^{i,j}$ from $p(y,z_{t+1:T}|x,z_{1:t-1},\tilde{z}^{i}_{t})$
	...
	\begin{columns}
		\begin{column}{0.5\linewidth}
		    \begin{itemize}
				...
				\item Use $r_{MC}$ to generate labels to train $r_{\psi}$
...
\begin{frame}{Formalized: Expert Iteration}
    ...
    %
    % I wrote this with the reward model involved, but technically it expert iteration doesn't
	...
	\begin{itemize}
		...
		\item Train $p(y,z|x)$, $r(z_t)$ on the labeled samples
	\end{itemize}
...
\begin{frame}{UCB for exploration}
    ...
    % we're unlikely to get the same z twice (and end up in the same state).
