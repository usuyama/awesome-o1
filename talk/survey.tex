...
\begin{frame}[t]{Scaling Laws for Board Games}
	\vspace{1cm}
	\begin{center}
		\includegraphics[width=0.9\linewidth]{images/Board}
	\end{center}
...
\begin{frame}[t]{Search Against Learned Verifiers}
	\vspace{1cm}
	\begin{center}
		\includegraphics[width=0.9\linewidth]{images/Verifiers}
	\end{center}
...
\begin{frame}{Sidenote: Do we have an Automatic Verifier?}
	\begin{columns}
		\begin{column}{0.5\linewidth}
			\begin{itemize}
				\item OpenAI primarily interested in learned verifiers (ORM)
				\item Speculation: Large-scale annotation of on-policy outputs
				\item Data collection to surmount search exploitation
			\end{itemize}
		\end{column}
		\begin{column}{0.5\linewidth}
			\includegraphics[width=\linewidth]{images/Verifiers2}
		\end{column}
	\end{columns}
...
\begin{frame}{Reinforcement Learning}
	I will mostly elide RL-specfic training question.
...
\begin{frame}{Early Guidance}
	\begin{columns}
		\begin{column}{0.5\linewidth}
			\begin{itemize}
				Early learned verification (PRM)
...
\begin{frame}{Framework: Beam Search with Guide}
	...
	\begin{columns}
		\begin{column}{0.5\linewidth}
			$$\cfr{g}: \cfz{\mathcal{S}^t} \rightarrow \mathbb{R}$$; Guide function
			\vspace{0.3cm}
...
\begin{frame}{Beam Search with Roll-Outs}
	\begin{columns}
		\begin{column}{0.5\linewidth}
			...
			$$\textcolor{c2}{g_{MC}}(\cfz{z_{t}}) = \frac{1}{N}\sum_{n=1}^{N} \Ver(\cy^{n})$$
		\end{column}
		\begin{column}{0.5\linewidth}
			\begin{tikzpicture}
				\foreach \i in {1,...,4}{
	...
	\begin{columns}
		\begin{column}{0.5\linewidth}
			\begin{itemize}
				...
				\item Learn $\textcolor{c2}{g_{\psi}}(\cfz{z_t})$ to approximate
				\item Use $\textcolor{c2}{g_{MC}}$ for labels
			\end{itemize}
		\end{column}
		\begin{column}{0.5\linewidth}
			\includegraphics[width=\textwidth]{images/mcroll.png}
...
\begin{frame}{Self-Evaluation // Generative Verifiers}
	\begin{itemize}
		\item Guidance from an LLM based on $\cfz{z_{1:t}}$
		\item Prompt to verify current correctness
		\item Note: allows CoT in the verifier
	\end{itemize}
	\blfootnote{\cite{Xie2023-ti,Zhang2024-sa}}
\end{frame}
...
\begin{frame}{Incorportating at Test}
	\begin{columns}
		\begin{column}{0.5\linewidth}
			\begin{itemize}
				\item Learned guide does not need $\Ver$
				      can be used at test.
				\item Alternative: trained into a single LLM
				\item \textit{Generative Verifier}
			\end{itemize}
		\end{column}
		\begin{column}{0.5\linewidth}
...
\begin{frame}{AlphaProof}
	...
	\begin{columns}
		\begin{column}{0.3\textwidth}
			\includegraphics[height=0.8\textheight]{gdm}
		\end{column}
		\begin{column}{0.6\textwidth}
			\begin{tcolorbox}[colback=white,colframe=black,boxrule=0.5pt]
...
\begin{frame}{Framework: Expert Iteration}
	...
	\begin{itemize}
		...
		\item Train $p(\cy,\cz|\cx)$, $g(\cz_t)$ on the labeled samples, and repeat
	\end{itemize}
...
\begin{frame}{Beyond Emergent Ablities}
	\begin{center}
		\includegraphics[width=0.8\linewidth]{images/gpt3i}
	\end{center}
\end{frame}

\begin{frame}{Interpretation outside of the Model}
	\begin{center}

	\end{center}
	\blfootnote{\cite{Kirchner2024-cu}}
\end{frame}

\begin{frame}{Evaluations Need to Be Much Harder}
	\begin{center}
		\includegraphic{}
	\end{center}
	\blfootnote{\url{https://commit-0.github.io/}}
\end{frame}

\begin{frame}{From Prompting to Specification}
	\begin{center}

	\end{center}
\end{frame}

\begin{frame}{Inference Time Systems}
	\begin{center}
